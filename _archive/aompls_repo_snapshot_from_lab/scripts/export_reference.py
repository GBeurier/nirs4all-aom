...
Outputs land in ``cpp/tests/reference/<DATASET>.json``.

This script is OPTIONAL — every binding in the repo ships with these fixtures
pre-generated. Re-run only when the upstream reference moves.

Run from the repo root::

    NIRS4ALL_BENCH=/path/to/nirs4all/bench/AOM_v0 python scripts/export_reference.py

NIRS4ALL_BENCH defaults to ``../nirs4all/nirs4all/bench/AOM_v0`` relative to
this repo (a sibling checkout layout).
"""
...
DEFAULT_NIRS4ALL = REPO_ROOT.parent / "nirs4all" / "nirs4all" / "bench" / "AOM_v0"
NIRS4ALL_BENCH = Path(os.environ.get("NIRS4ALL_BENCH", str(DEFAULT_NIRS4ALL)))

if not (NIRS4ALL_BENCH / "aompls" / "estimators.py").exists():
    raise SystemExit(
        f"Could not locate the nirs4all AOM_v0 reference at {NIRS4ALL_BENCH}. "
        "Set NIRS4ALL_BENCH to the bench/AOM_v0 directory of a nirs4all checkout."
    )

NIRS4ALL_ROOT = NIRS4ALL_BENCH.parents[1]  # …/nirs4all/nirs4all
sys.path.insert(0, str(NIRS4ALL_ROOT.parent))  # so `import nirs4all` works
sys.path.insert(0, str(NIRS4ALL_BENCH))
...
def _resolve_cohort_path(database: str, dataset: str) -> Tuple[Path, Path]:
    ...
    cohort_csv = NIRS4ALL_BENCH / "benchmarks" / "cohort_regression.csv"
    ...
    row = cohort.iloc[0]
    # cohort_regression.csv paths are relative to the nirs4all repo root.
    nirs4all_repo = NIRS4ALL_ROOT.parent
    return nirs4all_repo / row["train_path"], nirs4all_repo / row["ytrain_path"]
...
def _dump(out_path: Path, payload: Dict[str, Any]) -> None:
    ...
    print(f"  wrote {out_path.relative_to(REPO_ROOT)} ({size_mb:.2f} MiB)")


def main() -> int:
    out_dir = REPO_ROOT / "cpp" / "tests" / "reference"
