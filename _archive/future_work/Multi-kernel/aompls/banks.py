...
def asls_block_bank(p: Optional[int] = None, n_blocks: int = 5) -> List[LinearSpectralOperator]:
    """Bank of `n_blocks` block-mask operators on a `(n_blocks * p_block)`-
    wide augmented spectrum.

    Used in tandem with `aompls.preprocessing.ASLSBlockExpander` to wire
    the POP-ASLS-bank pipeline: the expander stacks `n_blocks` ASLS-corrected
    views of `X` side-by-side, the bank operators (one `BlockMaskOperator`
    per view) are strict-linear masks that preserve the AOM contract, and
    POP at the AOM level picks one view per PLS component.

    `p` here is the augmented total `n_blocks * p_block`. Pass `p=None`
    to build a "lazy" bank whose operators inherit `p` at fit time.
    """
    from .operators import BlockMaskOperator
    if p is None:
        # Use a placeholder block size of 1; the operators are rebound at
        # fit time via `fit_bank` once X is observed.
        return [BlockMaskOperator(block_index=b, n_blocks=n_blocks, block_size=1)
                for b in range(n_blocks)]
    if p % n_blocks != 0:
        raise ValueError(f"asls_block_bank: total p={p} must be a multiple of n_blocks={n_blocks}")
    block_size = p // n_blocks
    return [BlockMaskOperator(block_index=b, n_blocks=n_blocks, block_size=block_size)
            for b in range(n_blocks)]
...
def bank_by_name(name: str, p: Optional[int] = None) -> List[LinearSpectralOperator]:
    ...
    if name == "asls_block":
        # Default to 5 blocks (raw + 4 ASLS configs in ASLSBlockExpander)
        return asls_block_bank(p=p, n_blocks=5)
