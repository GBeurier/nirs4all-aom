...
class CriterionConfig:
    ...
    Attributes:
        kind: One of `"covariance"`, `"cv"`, `"approx_press"`, `"hybrid"`,
            `"holdout"`.
        cv: Number of folds for `cv` and `hybrid`. Default 5. Ignored if
            `cv_splitter` is provided.
        ...
        repeats: Number of CV repeats with different seeds. Default 1
            ...
            by sqrt(repeats). Only used when `kind == "cv"` AND
            `cv_splitter is None` (custom splitters are deterministic
            in our V1 — repeats > 1 with a custom splitter raises).
        ...
        cv_splitter: Optional sklearn-compatible cross-validator. If
            provided, replaces the random `KFold(n_splits=cv, shuffle=True,
            random_state=random_state)` construction in CV scoring. Must
            expose `.split(X, y, groups=None) -> Iterator[(train_idx,
            val_idx)]` and either `get_n_splits(X, y, groups=None)` or a
            `.n_splits` attribute. Examples:
            `KFold`, `StratifiedKFold`,
            `nirs4all.operators.splitters.SPXYFold`,
            `nirs4all.operators.splitters.KennardStoneSplitter`.
            Default None preserves the current KFold behavior bit-for-bit.
    """
...
def _resolve_cv_splitter(cv_splitter, n_splits_default: int, random_state: int):
    """Return (splitter, effective_n_splits) for CV scoring.

    If `cv_splitter` is None, build a fresh `KFold(n_splits_default,
    shuffle=True, random_state=random_state)` (current default path).
    Otherwise, use `cv_splitter` as-is and read its fold count from
    `get_n_splits()` (preferred) or `.n_splits` (fallback).
    """
    if cv_splitter is None:
        return KFold(n_splits=int(n_splits_default), shuffle=True, random_state=int(random_state)), int(n_splits_default)
    if hasattr(cv_splitter, "get_n_splits"):
        try:
            n_eff = int(cv_splitter.get_n_splits())
        except TypeError:
            # Some splitters require X / y to compute n_splits — try with None.
            n_eff = int(cv_splitter.get_n_splits(None, None, None))
    elif hasattr(cv_splitter, "n_splits"):
        n_eff = int(cv_splitter.n_splits)
    else:
        raise ValueError(
            f"cv_splitter {type(cv_splitter).__name__} must expose get_n_splits() or .n_splits"
        )
    if n_eff < 2:
        raise ValueError(f"cv_splitter must yield at least 2 folds, got {n_eff}")
    return cv_splitter, n_eff
...
def cv_score_regression(
    ...
    If `cv_splitter` is provided, it replaces the default
    `KFold(n_splits, shuffle=True, random_state=random_state)`. The
    splitter's `get_n_splits()` / `.n_splits` is honoured.
    ...
    """
    splitter, _ = _resolve_cv_splitter(cv_splitter, n_splits, random_state)
    rmses = []
