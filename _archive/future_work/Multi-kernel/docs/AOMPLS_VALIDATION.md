...
3. The smoke benchmark numbers from
   `bench/aom_v0/Multi-kernel/benchmark_runs/smoke/results.csv` (regression) and
   `bench/aom_v0/Multi-kernel/benchmark_runs/smoke/results_classification.csv`
   (classification).
...
The test suite lives in `bench/aom_v0/Multi-kernel/tests/`. As of phase 9, **72 tests are
...
`python -m benchmarks.run_smoke_benchmark` from `bench/aom_v0/Multi-kernel/`. It runs
...
- `bench/aom_v0/Multi-kernel/benchmark_runs/smoke/results.csv` (33 regression rows)
- `bench/aom_v0/Multi-kernel/benchmark_runs/smoke/results_classification.csv` (10 classification rows)
...
The full benchmark runner is `bench/aom_v0/Multi-kernel/benchmarks/run_aompls_benchmark.py`.
...
`bench/aom_v0/Multi-kernel/benchmarks/build_cohorts.py`:

- `bench/aom_v0/Multi-kernel/benchmarks/cohort_regression.csv`
- `bench/aom_v0/Multi-kernel/benchmarks/cohort_classification.csv`

### Regression command

From `bench/aom_v0/Multi-kernel/`:

```bash
python -m benchmarks.run_aompls_benchmark \
    --task regression \
    --cohort bench/aom_v0/Multi-kernel/benchmarks/cohort_regression.csv \
    --master bench/tabpfn_paper/master_results.csv \
    --workspace bench/aom_v0/Multi-kernel/benchmark_runs/full_regression \
    --seeds 0,1,2 \
    --n-jobs 1 \
    --criterion cv \
    --max-components 25 \
...
python -m benchmarks.run_aompls_benchmark \
    --task classification \
    --cohort bench/aom_v0/Multi-kernel/benchmarks/cohort_classification.csv \
    --workspace bench/aom_v0/Multi-kernel/benchmark_runs/full_classification \
    --seeds 0,1,2 \
    --n-jobs 1 \
    --criterion cv \
    --max-components 15 \
...
The smoke runner is `bench/aom_v0/Multi-kernel/benchmarks/run_smoke_benchmark.py`. It
...
`bench/aom_v0/Multi-kernel/benchmark_runs/smoke/` are produced by:

```bash
cd bench/aom_v0/Multi-kernel
...
`bench/aom_v0/Multi-kernel/docs/codex_review_prompts/` and ready to run:
...
`bench/aom_v0/Multi-kernel/docs/CODEX_REVIEWS.md` (one section per prompt) once each
...
in `bench/aom_v0/Multi-kernel/benchmark_runs/smoke/results_classification.csv`. The
...
Output: `bench/aom_v0/Multi-kernel/benchmark_runs/extended/results.csv`. Workspace is
...
`bench/aom_v0/Multi-kernel/publication/tables/summary_per_dataset.csv`.
