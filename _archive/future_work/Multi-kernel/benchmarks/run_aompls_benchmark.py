...
def _build_preproc(name: str):
    ...
    # ASLS hardness presets (lambda controls baseline rigidity, p controls
    # asymmetry). soft = rigid baseline, less correction. hard = soft
    # baseline, more correction. medium = the canonical default.
    if name == "asls_soft":
        return ASLSBaseline(lam=1e3, p=0.001)
    if name == "asls_medium":
        return ASLSBaseline(lam=1e5, p=0.01)
    if name == "asls_hard":
        return ASLSBaseline(lam=1e7, p=0.05)
    # Partial-alpha ASLS: subtract only `alpha` of the estimated baseline.
    # Uses the medium ASLS configuration (lam=1e6, p=0.01 — same as the
    # default ASLSBaseline used in the `none+asls+none` preproc grid).
    if name.startswith("asls_alpha_"):
        from aompls.preprocessing import PartialASLSBaseline
        try:
            alpha_str = name[len("asls_alpha_"):]
            alpha = float(alpha_str.replace("p", "."))
        except ValueError:
            raise ValueError(f"could not parse alpha from {name!r}")
        return PartialASLSBaseline(lam=1e6, p=0.01, alpha=alpha)
    # ASLS block expansion: stack [X | ASLS_soft(X) | ASLS_med(X) |
    # ASLS_default(X) | ASLS_hard(X)] for the POP-ASLS-bank pipeline.
    if name == "asls_block_expand":
        from aompls.preprocessing import ASLSBlockExpander
        return ASLSBlockExpander()
    if "+" in name:
...
def _build_estimator(variant: Dict, criterion: str, max_components: int, cv: int, seed: int, X=None, y=None):
    ...
    # Optional custom CV splitter. The variant config carries a
    # `cv_splitter_factory` callable `(seed: int) -> splitter` so that
    # the splitter is instantiated lazily and the benchmark resume key
    # (variant label) carries every parameter that affects results.
...
def _run_regression_variant(
    ...
    return {
        ...
        "run_seed": seed,
        "code_version": "aom_v0/Multi-kernel/0.1.0",
