...
Output: `bench/aom_v0/Multi-kernel/benchmark_runs/extended/results.csv` (resumable).
...
# ---------------------------------------------------------------------------
# SPXYFold variants for the top-3 fixed-config champions: replace the random
# KFold criterion with a Kennard-Stone-style joint X-Y splitter
# (`nirs4all.operators.splitters.SPXYFold`). The hypothesis is that a
# chemistry-aware splitter reduces selection variance better than HPO does
# (HPO did not beat the fixed-config baselines on this cohort).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Pure SPXYFold variants (no preprocessing) to isolate the splitter effect
# from the ASLS gain. The original SPXY_VARIANTS above all carry ASLS in
# `preproc`, so the comparison "SPXY vs random KFold" is confounded with
# "ASLS vs no preproc". These variants replicate the structure without
# the ASLS step so we can attribute the gain (or lack of) to the splitter
# alone.
# ---------------------------------------------------------------------------
SPXY_PURE_VARIANTS = [
    {"label": "SPXYpure-AOM-compact-cv5-numpy", "kind": "regression", "selection": "global", "engine": "nipals_adjoint", "operator_bank": "compact", "backend": "numpy", "criterion_override": "cv", "cv_override": 5, "cv_splitter_factory": _spxy_factory(5)},
    {"label": "SPXYpure-AOM-family-pruned-cv3-numpy", "kind": "regression", "selection": "global", "engine": "nipals_adjoint", "operator_bank": "family_pruned", "backend": "numpy", "criterion_override": "cv", "cv_override": 3, "cv_splitter_factory": _spxy_factory(3)},
    {"label": "SPXYpure-AOM-response-dedup-cv3-numpy", "kind": "regression", "selection": "global", "engine": "nipals_adjoint", "operator_bank": "response_dedup", "backend": "numpy", "criterion_override": "cv", "cv_override": 3, "cv_splitter_factory": _spxy_factory(3)},
]
EXT_REG_VARIANTS = EXT_REG_VARIANTS + SPXY_PURE_VARIANTS


# ---------------------------------------------------------------------------
# ASLS hybrid variants (Levels A and B from the design plan).
# Level B: explicit soft / medium / hard ASLS configurations on the
# compact bank with CV-5 (the winning recipe). Lambda controls baseline
# rigidity; p controls asymmetry. Soft = rigid baseline + low p (gentle
# correction), hard = soft baseline + high p (aggressive correction),
# medium = canonical default.
# Level A: partial-alpha ASLS — subtract only `alpha` of the estimated
# baseline. alpha < 1 keeps part of the baseline as informative signal,
# alpha > 1 over-corrects.
# ---------------------------------------------------------------------------
ASLS_HYBRID_VARIANTS = [
    # Level B — hardness ablation
    {"label": "ASLSsoft-AOM-compact-cv5-numpy", "kind": "regression", "selection": "global", "engine": "nipals_adjoint", "operator_bank": "compact", "backend": "numpy", "preproc": "asls_soft", "criterion_override": "cv", "cv_override": 5},
    {"label": "ASLSmed-AOM-compact-cv5-numpy", "kind": "regression", "selection": "global", "engine": "nipals_adjoint", "operator_bank": "compact", "backend": "numpy", "preproc": "asls_medium", "criterion_override": "cv", "cv_override": 5},
    {"label": "ASLShard-AOM-compact-cv5-numpy", "kind": "regression", "selection": "global", "engine": "nipals_adjoint", "operator_bank": "compact", "backend": "numpy", "preproc": "asls_hard", "criterion_override": "cv", "cv_override": 5},
    # Level A — alpha blending (partial subtraction of baseline)
    {"label": "ASLSalpha050-AOM-compact-cv5-numpy", "kind": "regression", "selection": "global", "engine": "nipals_adjoint", "operator_bank": "compact", "backend": "numpy", "preproc": "asls_alpha_0p5", "criterion_override": "cv", "cv_override": 5},
    {"label": "ASLSalpha075-AOM-compact-cv5-numpy", "kind": "regression", "selection": "global", "engine": "nipals_adjoint", "operator_bank": "compact", "backend": "numpy", "preproc": "asls_alpha_0p75", "criterion_override": "cv", "cv_override": 5},
    {"label": "ASLSalpha125-AOM-compact-cv5-numpy", "kind": "regression", "selection": "global", "engine": "nipals_adjoint", "operator_bank": "compact", "backend": "numpy", "preproc": "asls_alpha_1p25", "criterion_override": "cv", "cv_override": 5},
]
EXT_REG_VARIANTS = EXT_REG_VARIANTS + ASLS_HYBRID_VARIANTS


# ---------------------------------------------------------------------------
# Level C - POP-ASLS-bank prototype: stack [raw | ASLS_soft | ASLS_med |
# ASLS_default | ASLS_hard] views into a (5p)-augmented matrix and let
# AOM-global pick the best view by CV at fit time.
# ---------------------------------------------------------------------------
ASLS_BLOCK_VARIANTS = [
    {"label": "AOM-aslsblock-cv5-numpy", "kind": "regression", "selection": "global", "engine": "nipals_materialized", "operator_bank": "asls_block", "backend": "numpy", "preproc": "asls_block_expand", "criterion_override": "cv", "cv_override": 5},
    {"label": "POP-aslsblock-cv5-numpy", "kind": "regression", "selection": "per_component", "engine": "nipals_materialized", "operator_bank": "asls_block", "backend": "numpy", "preproc": "asls_block_expand", "criterion_override": "cv", "cv_override": 5},
]
EXT_REG_VARIANTS = EXT_REG_VARIANTS + ASLS_BLOCK_VARIANTS
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--workspace", default="bench/aom_v0/Multi-kernel/benchmark_runs/extended")
    ...
    cohort_reg_path = "bench/aom_v0/Multi-kernel/benchmarks/cohort_regression.csv"
    cohort_clf_path = "bench/aom_v0/Multi-kernel/benchmarks/cohort_classification.csv"
