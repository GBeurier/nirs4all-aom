"""Phase 11 full-57: adaptive super-learner + trimmed mean."""
...
from run_smoke4_phase11 import VARIANTS  # noqa: E402
...
def main(argv=None) -> int:
    ...
    print(f"[phase11-full57] {len(valid)} datasets x {len(VARIANTS)} variants")
    ...
    for _, cohort_row in valid.iterrows():
        try:
            ...
            print(f"[phase11-full57] ERROR loading {cohort_row['dataset']}: {exc}")
            continue
        for label, runner in VARIANTS:
            ...
            n_appended += 1
            print(
                f"[phase11-full57] {cohort_row['dataset']:<48s} {label:<26s} "
                f"rmsep={row.get('rmsep', 'NA')} t={row.get('fit_time_s', 'NA')}"
            )

    print(f"[phase11-full57] appended {n_appended} rows")
    return 0
