"""Phase 10 full-57: mean ensembles built from Phase 9 winners.

Variants:
- mean-ensemble-3-fixed: avg of {multiK-3-5-7 fixed=10, moe-preproc-soft, lazy-V2-AOM}
- mean-ensemble-4-fixed: + AOM-PLS-compact
- moe-view-multiK-wide-2-10 (wider K, auto components)
...
def _runner_mean_ensemble_3_fixed(Xtr, ytr, Xte, yte, seed, max_components):
    p = Xtr.shape[1]
    bases = [
...
def _runner_mean_ensemble_4_fixed(Xtr, ytr, Xte, yte, seed, max_components):
    p = Xtr.shape[1]
    bases = [
        ("multiK-3-5-7", AOMMoEMultiK(
            K_list=(3, 5, 7), per_expert_components=10, random_state=seed,
...
VARIANTS = [
    ("mean-ensemble-3-fixed", _runner_mean_ensemble_3_fixed),
    ("mean-ensemble-4-fixed", _runner_mean_ensemble_4_fixed),
...
def main(argv=None) -> int:
    ...
    print(f"[phase10-full57] {len(valid)} datasets x {len(VARIANTS)} variants")
    ...
    for _, cohort_row in valid.iterrows():
        try:
            ...
            print(f"[phase10-full57] ERROR loading {cohort_row['dataset']}: {exc}")
            continue
        for label, runner in VARIANTS:
            ...
            n_appended += 1
            print(
                f"[phase10-full57] {cohort_row['dataset']:<48s} {label:<26s} "
                f"rmsep={row.get('rmsep', 'NA')} t={row.get('fit_time_s', 'NA')}"
            )

    print(f"[phase10-full57] appended {n_appended} rows")
    return 0
