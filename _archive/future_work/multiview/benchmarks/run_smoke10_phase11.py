...
def main(argv=None) -> int:
    ...
    print(f"[phase11-smoke10] {len(smoke)} datasets x {len(VARIANTS)} variants")
    ...
    for _, cohort_row in smoke.iterrows():
        ...
        for label, runner in VARIANTS:
            ...
            n_appended += 1
            print(
                f"[phase11-smoke10] {cohort_row['dataset']:<48s} {label:<28s} "
                f"rmsep={row.get('rmsep', 'NA')} t={row.get('fit_time_s', 'NA')}"
            )

    print(f"[phase11-smoke10] appended {n_appended} rows")
    return 0
