"""D-A-001 audit20 paired statistics computation for Codex round-8 GATE.
...
Per Codex round-7 verdict (SYNC.md 2026-05-07 03:30 CEST D-A-001 LOCKED_SCOPED):
  audit20 cohort follows the same protocol as fast12, with cohort cardinality
  scaled to N=20 datasets (60 rows per candidate). Selectors and baselines
  are unchanged.
  ...
  - Primary unit: dataset-level seed mean (N=20) ; sensitivity: row-level (N=60)
...
Usage: python d_a_001_audit20_paired_stats.py
"""
...
RUN_DIR = HERE.parent / "benchmark_runs" / "da001_audit20_seeds012"
RESULTS_CSV = RUN_DIR / "results.csv"
DOCS_DIR = HERE.parent / "docs"
OUT_CSV = DOCS_DIR / "D_A_001_audit20_paired_stats.csv"
OUT_MD = DOCS_DIR / "D_A_001_AUDIT20_PAIRED_STATS.md"
...
def compute_pair_stats(df: pd.DataFrame, target: str, baseline: str) -> dict:
    ...
    # Per-dataset means (primary unit per Codex round-6/7).
    by_ds = (
...
def _verdict(row: dict) -> str:
    """Apply the Codex round-6/7 multi-criteria gate.
...
def main() -> None:
        ...
        rows.append(
            {
                ...
                "wins_rows_60": s.get("wins_rows"),
                "wins_datasets_20": s.get("wins_datasets"),
    ...
    md_lines: list[str] = []
    md_lines.append("# D-A-001 audit20 paired statistics (Codex round-8 GATE)")
    md_lines.append("")
    md_lines.append(
    ...
    md_lines.append("")
    md_lines.append(
        f"Cohort: audit20_transfer_core x seeds 0/1/2 ({N_DATASETS} datasets x 3 seeds = {N_ROWS} rows per candidate)."
    )
    md_lines.append("")
    md_lines.append(
        "Wilcoxon = paired one-sided (target < baseline) on log RMSEP deltas. "
        f"Primary unit = per-dataset seed-mean (N={N_DATASETS}). Row-level (N={N_ROWS}) reported "
        "as sensitivity. Holm correction across the 4 x 2 = 8 comparisons."
    )
    md_lines.append("")
    md_lines.append("Win threshold conventions (from Codex round-6/7):")
    ...
    md_lines.append("")
    header = (
        f"| Selector | Baseline | N_ds | Wins/{N_DATASETS} | Wins/{N_ROWS} | "
    ...
    for selector in SELECTORS:
        ...
        for baseline in BASELINES:
            ...
            md_lines.append("")
            md_lines.append(
                f"- Rows kept: {s.get('n_rows', 0)} (out of {N_ROWS} per side); "
                f"datasets kept: {s.get('n_datasets', 0)} (out of {N_DATASETS})"
            )
            md_lines.append(
                f"- Wins (per-row, N={N_ROWS}): {s.get('wins_rows')}/{s.get('n_rows', 0)} ; "
                f"Wins (per-dataset, N={N_DATASETS}): {s.get('wins_datasets')}/{s.get('n_datasets', 0)}"
            )
            md_lines.append(
    ...
    wide = wide.dropna()
    md_lines.append("")
    md_lines.append(f"Rows with all 5 variants OK: {len(wide)} (out of {N_ROWS} possible).")
    if len(wide) >= 6:
    ...
    md_lines.append(
        ...
        "but out of scope for this audit20 GATE deliverable."
    )
