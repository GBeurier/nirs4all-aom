...
Best FastAOM exploration family in the full60 table.\\
...
The refreshed manifest contains 61 regression rows and 17 classification rows.
The main regression comparisons use the strict intersection
$N_{\cap}=32$; the per-dataset long table below includes only those rows.
The classification tables are paired within the classification cohort.
...
\begin{table}[h]
\centering
\caption{FastAOM full-variant summary from the full60 seed0 table.}
...
representation.  FastAOM was run on seed0 for the full60 exploration and
...
computed from benchmark result tables produced by the public repositories
announced in the main manuscript.  The regression cohort contains 61 manifest
rows, and the main paired regression comparisons use $N_{\cap}=32$ rows.  The
classification cohort contains 17 manifest rows and is summarized in the
...
AI assistance was used to draft explanatory text and to generate
manuscript-support scripts for aggregation, paired statistics and figure
generation.  Every numerical claim in the main manuscript and in this
supplement is computed from benchmark result tables; the assistance did not
generate numerical results by itself.  The authors remain responsible for the
study design, data interpretation, code review and final text.
